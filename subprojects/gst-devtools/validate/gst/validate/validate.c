...
static gboolean
_set_feature_rank (GQuark field_id, const GValue * value,
...
gst_validate_setup_test_file (const gchar * testfile, gboolean use_fakesinks)
{
  ...
  for (GList * tmp = feature_ranks_def; tmp; tmp = tmp->next) {
    ...
    gst_structure_foreach (feature_ranks,
        (GstStructureForeachFunc) _set_feature_rank, feature_ranks);
  }
