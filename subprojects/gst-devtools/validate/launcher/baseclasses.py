                ...
                if self.options.gdb:
                    ...
                    while True:
                        try:
                            ...
                            if not debugger:
                                input(f"{Colors.FAIL}Timeout happened on {self.classname} no known debugger found but the process PID is {self.process.pid}\n"
                                    f"You can find log file at {self.logfile}\n"
                                    f"Press enter to continue\n\n")
                            else:
                                input(f"{Colors.FAIL}Timeout happened on {self.classname} you can attach the debugger doing:\n"
                                      f"   $ {' '.join([shlex.quote(a) for a in debugger.get_attach_args(self.process.pid)])}\n"
                                      f"You can find log file at {self.logfile}\n"
                                      f"**Press enter to continue**{Colors.ENDC}\n\n")
